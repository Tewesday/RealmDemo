...
TArray<FMassEntityHandle> UOctreeSys::GetEntitiesInBox(const FBoxCenterAndExtent& QueryBox)
{
	...
	TArray<int> ResultIDs;

	QuadTree.RangeQuery(UE::Geometry::FAxisAlignedBox3d(AABB), ResultIDs);
	...
	for (int32 index = 0; index < EntityHandles.Num(); ++index) {
		...
		for (int32 IDsIndex = 0; IDsIndex < ResultIDs.Num(); ++IDsIndex) {
			if (OctreeFragment->OctreeID == ResultIDs[IDsIndex]) {
